...
class SkillContractTests(unittest.TestCase):
    ...
    def test_skill_documents_fail_closed_helpers_and_mode_axes(self) -> None:
        ...
        for phrase in (
            ...
            "Treat `when`, `if`, `unless`",
            "enumerate every absent required input",
            "For each ambiguity, give a stable ID",
            "each materially",
            "specific authorized technical decision role",
            "human reviewers with different identifiers",
            "must approve the same exact final",
